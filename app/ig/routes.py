...
#
#
#   NEED TO pip install flask-cors
#   before api will work because we need to allow access to our flask app
#   and add from flask_cors import CORS to __init__.py because we need 
#
#
...
def apiPosts():
    ...
    return {
        ...
        'posts': [p.to_dict() for p in posts] # We cannot just say 'posts': posts because it will give an error, so we create a to_dict() method in our Post class in models.py and convert the values into a dictionary so we can call it here
        }
...
@ig.route('/api/update-post', methods=["POST"])
@login_required
def apiUpdatePost(user):
    data = request.json
    ...
    # db.session.add(post) <-- Do not need to add because we are just updating
    db.session.commit()   
